...
describe('dsh-tool-subagent', () => {
  ...
  it('treats an unknown (plugin-added) stop reason as an isError result', async () => {
    ...
    ctx.subagents.registerProvider({
      name: 'weird',
      capabilities: { outputSchema: false, depthLimit: false, toolFilter: false, persona: false, reasoningEffort: false },
  ...
  it('passes persona/toolFilter/maxDepth config through to the start request', async () => {
    ...
    ctx.subagents.registerProvider({
      name: 'capture2',
      capabilities: { outputSchema: false, depthLimit: true, toolFilter: true, persona: true, reasoningEffort: true },
      inheritsParentContext: false,
      start: async (request) => {
        seen = request
        return {
  ...
  it('a partial toolFilter (deny only) does not materialize an empty allow-list (deny-all trap)', async () => {
    ...
    ctx.subagents.registerProvider({
      name: 'capture3',
      capabilities: { outputSchema: false, depthLimit: false, toolFilter: true, persona: false, reasoningEffort: false },
      inheritsParentContext: false,
      start: async (request) => {
        seen = request
        return {
