 ...
 * (the scoped structured runtime), and `toolFilter`/`persona`/`reasoningEffort`
 * (scoped `restrict()`, a scoped shadowing persona section, and a scoped
 * effort-pinning request listener, applied in the child's creation window).
 */
class SpawnInProcessProvider implements SubagentProvider {
  readonly capabilities: SubagentCapabilities = { outputSchema: true, depthLimit: true, toolFilter: true, persona: true, reasoningEffort: true }
